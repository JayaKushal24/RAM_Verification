        ...
        task drv_run;
                forever begin
                        if(vif.rst) begin
                                ...
                                vif.read_en<=t1.read_en;
                                vif.write_en<=t1.write_en;
                                vif.transaction_count<=t1.transaction_count;
                                $display("[DRV sent:%0d] @%0t read=%0d write=%0d addr=%0d data_in=%0d",t1.transaction_count,$time,t1.read_en,t1.write_en,t1.addr,t1.data_in);
                                @(posedge vif.clk);
                                drv_ref.put(t2);
                        end
                end
